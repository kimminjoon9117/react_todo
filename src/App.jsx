  ...
  useEffect(() => {
    if (currentTodo) {
      fetch(`http://localhost:3000/todo/${currentTodo}`, {
        ...
        setTodo(prev => prev.map(el => el.id === currentTodo ? res : el)))
    }
  }, [time])
...
const TodoInput = ({ setTodo }) => {
  ...
  const addTodo = () => {
    ...
    }).then(res => res.json()).then(res => setTodo((prev) => [...prev, res]))
  };
...
const formatTime = (seconds) => {
  const timeString = `${String(Math.floor(seconds / 3600)).padStart(2, "0")}:${String(Math.floor((
    seconds % 3600) / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`;
  return timeString;
